...
public class GraphicalMode extends GameUI {
    ...
    public GraphicalMode() {
        ...
        chipImage = new ImageIcon(getClass().getResource("/assets/images/chip/chip.png")).getImage();
    	backgroundImage = new ImageIcon(getClass().getResource("/assets/images/backgrounds/table_background.jpg")).getImage();
    }
    
    public ImageIcon getImageCard(String image) {
        String filepath = "/assets/images/cards/" + image + ".png";
	...
	@Override
	public void drawHidingStateVerticalBaCay(ControlGame control, int x) {
	    player = control.getPlayer();
	    // Length of player's name
	    int length = (int)g2.getFontMetrics(getFont().deriveFont(35F)).stringWidth(player.getName());
	    int w = (WIDTH_CARD > length ? WIDTH_CARD : length) + 50;
	    ...
	    // Điều chỉnh vị trí dựa trên turn để tránh chồng lấp
	    if (control.getTurn() == 1) { // Người chơi bên phải
	        x = control.getWidth() - w - 30;
	    } else if (control.getTurn() == 3) { // Người chơi bên trái
	        x = 30;
	    }
	    ...
	    g2.setFont(getFont().deriveFont(28F));
	    length = (int)g2.getFontMetrics(getFont().deriveFont(28F)).stringWidth(player.getNumCards()+"");
	    g2.drawString(player.getNumCards()+"", w/2-length/2+x, y+h-35);
	    
	    // Vẽ chip và số tiền cược
	    int chipSize = 60;
	    int chipX, chipY;
	    
	    // Vẽ chip hướng vào trong (theo chiều dọc)
	    if (control.getTurn() == 1) { // Người chơi bên phải
	        chipX = x - chipSize - 10; // Đặt chip bên trái của panel
	    } else { // Người chơi bên trái (3)
	        chipX = x + w + 10; // Đặt chip bên phải của panel
	    }
	    chipY = y + h/2 - chipSize/2;
	    
	    // Vẽ chip
	    g2.drawImage(chipImage, chipX, chipY, chipSize, chipSize, null);
	    
	    // Vẽ số tiền cược
	    g2.setColor(Color.WHITE);
	    g2.setFont(new Font("Arial", Font.BOLD, 18));
	    String betAmount = Integer.toString(player.getBet());
	    int betWidth = g2.getFontMetrics().stringWidth(betAmount);
	    g2.drawString(betAmount, chipX + chipSize/2 - betWidth/2, chipY + chipSize/2 + 6);
	}

	/**
	 * Vẽ trạng thái ẩn theo chiều ngang cho game BaCay
	 */
	@Override
	public void drawHidingStateHorizontalBaCay(ControlGame control, int y) {
	    player = control.getPlayer();
	    // length of player's name
	    int length = (int)g2.getFontMetrics(getFont().deriveFont(35F)).stringWidth(player.getName());
	    int w = length + WIDTH_CARD + 60;
	    ...
	    // Điều chỉnh vị trí dựa trên turn để tránh chồng lấp
	    if (control.getTurn() == 0) { // Người chơi trên cùng
	        y = 30;
	    } else if (control.getTurn() == 2) { // Người chơi dưới cùng
	        y = control.getHeight() - h - 30;
	    }
	    ...
	    // Draw player's name
	    g2.setFont(getFont().deriveFont(35F));
	    g2.drawString(player.getName(), (w+15+WIDTH_CARD-length)/2+x, y+h/2 + 10);
	    
	    // Draw back card
	    g2.drawImage(getImageCard("cardback").getImage(), x+15, y+(h-HEIGHT_CARD)/2, WIDTH_CARD, HEIGHT_CARD, null);
	    
	    // Draw the number of cards remaining (không cần cho BaCay vì luôn là 3 lá)
	    
	    // Vẽ chip và số tiền cược
	    int chipSize = 60;
	    int chipX, chipY;
	    
	    // Vẽ chip hướng vào trong (theo chiều ngang)
	    if (control.getTurn() == 0) { // Người chơi trên cùng
	        chipX = x + w/2 - chipSize/2;
	        chipY = y + h + 10; // Đặt chip bên dưới panel
	    } else { // Người chơi dưới cùng (2)
	        chipX = x + w/2 - chipSize/2;
	        chipY = y - chipSize - 10; // Đặt chip bên trên panel
	    }
	...
	public void drawHidingStateBaCay(ControlGame control, int x, int y) {
		int savedTurn = control.getTurn();
		int numPlayers = control.getNumPlayers();
        if(numPlayers >= 1) {
        	control.setTurn(0);
        	drawHidingStateHorizontalBaCay(control, y);
        }
        if(numPlayers >= 2) {
        	control.setTurn(1);
        	player = control.getPlayer();
        	int w = (int)g2.getFontMetrics(getFont().deriveFont(35F)).stringWidth(player.getName());
        	w = (WIDTH_CARD > w ? WIDTH_CARD : w) + 75;
        	drawHidingStateVerticalBaCay(control, control.getWidth()-w);
        }
        if(numPlayers >= 3) {
        	control.setTurn(2);
        	drawHidingStateHorizontalBaCay(control, control.getHeight()-195);
        }
        if(numPlayers == 4){
        	control.setTurn(3);
        	drawHidingStateVerticalBaCay(control, x);
        }
        control.setTurn(savedTurn);
	}
