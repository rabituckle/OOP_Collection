...
public class BasicMode extends GameUI {
    ...
    //fix kích thước
    private static final int CARD_SIZE = 45; //size of square ~ a card 
    private static final int SPACING = 2; //space between 2 cards
    private Map<Card, Rectangle> mapCard; //map:1 rectangle ~ 1 card
    private ArrayList<Integer> cardState; //state of card 
    ...
    public void drawHidingStateVertical(ControlGame control, int x) {
        ...
        g2.drawString(player.getName(), x - 5, y - 5);
    ...
    public void drawHidingStateHorizontal(ControlGame control, int y) {
        ...
        g2.drawString(playerName, x + (totalWidth - nameWidth) / 2, y - 10);
        
        //card back
        for (int i = 0; i < numCards; i++) {
            drawSimpleCardBack(x + i * (CARD_SIZE + SPACING), y, CARD_SIZE, CARD_SIZE);
        }
    }
