...
public class BaCay extends ControlGame implements Runnable {
    ...
    public void paintComponent(Graphics g) {
        ...
        for(int i = 0; i < getNumPlayers(); i++) {
        	setTurn(i);
            if (getTurn() == 0) { // Người chơi trên cùng
            	int length = (int)GameUI.g2.getFontMetrics(getFont().deriveFont(35F)).stringWidth(getPlayer().getName());
        		int w = length + GameUI.WIDTH_CARD + 60;
            	int h = GameUI.HEIGHT_CARD + 30;
            	w = this.getWidth()/2 + w/2;
                ui.drawChip(this, w - 20, h - 15);
            }
            else if (getTurn() == 1) { // Người chơi bên phải
            	int length = (int)GameUI.g2.getFontMetrics(getFont().deriveFont(35F)).stringWidth(getPlayer().getName());
        		int w = (GameUI.WIDTH_CARD > length ? GameUI.WIDTH_CARD : length) + 50;
        		int h = 210;
        		w = this.getWidth() - w/2;
        		h = this.getHeight()/2 + h/2;
                ui.drawChip(this, w, h - 10);
            }
            else if (getTurn() == 2) { // Người chơi dưới cùng
            	int length = (int)GameUI.g2.getFontMetrics(getFont().deriveFont(35F)).stringWidth(getPlayer().getName());
        		int w = length + GameUI.WIDTH_CARD + 60;
