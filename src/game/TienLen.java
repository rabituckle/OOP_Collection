...
public abstract class TienLen extends ControlGame{
	...
	private void changeScore() {
		...
		for(int i = 0; i < getNumPlayers(); i++) {
			if(player.get(i).getName().equals(firstTurn)) {
				player.get(i).setScore(player.get(i).getScore() + maxNumCards*10);
			}
			else{
				int score = player.get(i).getScore() - player.get(i).getNumCards()*10;
				player.get(i).setScore(score);
			}
		}
	}
	
	protected void increaseNumPass() {
		numPass++;
	}
