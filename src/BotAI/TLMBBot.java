...
public class TLMBBot {
	...
	public ArrayList<Card> findConsecutivePairs(ArrayList<Card> playerCard, ArrayList<Card> pre){
		...
		if(pre == null) {
			...
			while(j < playerCard.size() && playerCard.get(j).getCost() < 15) {
				if(playerCard.get(i).getCost() == playerCard.get(j).getCost()
						&& playerCard.get(i).getColor() == playerCard.get(j).getColor()) {
					if(ans.size() >= 6) {
						if(ans.get(ans.size()-1).getCost() + 1 < playerCard.get(i).getCost()
								|| ans.get(ans.size()-1).getColor() == playerCard.get(i).getColor()) {
					...
					if(ans.size() != 0 && ans.get(ans.size()-1).getColor() == playerCard.get(i).getColor()) {
						ans.clear();
					}
		...
		int i = 0;
		while(playerCard.get(i).getCost() < pre.get(0).getCost() || playerCard.get(i).getColor() == pre.get(0).getColor()) {
			i++;
		...
		while(j < playerCard.size() && playerCard.get(j).getCost() < 15) {
			if(playerCard.get(i).getCost() == playerCard.get(j).getCost()
					&& playerCard.get(i).getColor() == playerCard.get(j).getColor()
					&& playerCard.get(i).getColor() == pre.get(0).getColor()) {
		...
		if(ans.get(ans.size()-1).compare(pre.get(pre.size()-1)) < 0) {
			return null;
		}
		deleteCard(playerCard, ans);
		return ans;
	...
	public ArrayList<Card> engineSingle(ArrayList<Card> playerCard, ArrayList<Card> pre){
		...
		for(int i = 0; i < len; i++) {
			if(playerCard.get(i).compare(pre.get(0)) > 0 && playerCard.get(i).getSuit().equals(pre.get(0).getSuit())) {
				ans.add(playerCard.get(i));
				playerCard.remove(i);
				return ans;
			}
			// Neu playerCard[i] la heo thi chat dc moi la le nho hon
			if(playerCard.get(i).compare(pre.get(0)) > 0 && playerCard.get(i).getCost() == 15) {
				ans.add(playerCard.get(i));
				playerCard.remove(i);
				return ans;
			}
	...
	public ArrayList<Card> engineDouble(ArrayList<Card> playerCard, ArrayList<Card> pre){
		...
		while(i < playerCard.size() - 1) {
			if(playerCard.get(i).getCost() == playerCard.get(i+1).getCost()
					&& playerCard.get(i).getColor() == playerCard.get(i+1).getColor()) {
				if(playerCard.get(i+1).compare(pre.get(1)) > 0 && playerCard.get(i).getColor() == pre.get(0).getColor()) {
					ans.add(playerCard.get(i));
					playerCard.remove(i);
					ans.add(playerCard.get(i));
					playerCard.remove(i);
				...
				if(playerCard.get(i).getCost() == 15 && playerCard.get(i+1).getCost() == 15
						&& playerCard.get(i).getColor() == playerCard.get(i+1).getColor()) {
					if(playerCard.get(i+1).compare(pre.get(1)) > 0) {
						ans.add(playerCard.get(i));
						playerCard.remove(i);
						ans.add(playerCard.get(i));
						playerCard.remove(i);
	...
	public ArrayList<Card> engineSequence(ArrayList<Card> playerCard, ArrayList<Card> pre){
		...
		int len = pre.size();
		int i = 0;
		while(playerCard.get(i).compare(pre.get(0)) < 0 || !playerCard.get(i).getSuit().equals(pre.get(0).getSuit())) {
			i++;
			if(i == playerCard.size() || playerCard.get(i).getCost() == 15) {
				return null;
			}
